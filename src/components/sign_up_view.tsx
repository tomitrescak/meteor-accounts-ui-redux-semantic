...
export default class SignUp extends React.PureComponent<IRegistrationComponent, {}> {
  ...
  register(e: any) {
    ...
    this.props.state.register(this.props.state, name, this.email ? this.email.toLowerCase() : '', this.password1, this.password2, currentState.profileData, toJS(this.profile));
  }
  ...
  render() {
    ...
    return (
      <Form onSubmit={this.register.bind(this)} method="post" className={this.props.inverted ? 'inverted' : ''}>
        <Form.Input label={i18n`Name and Surname`} placeholder={i18n`Your full name`} name="name" icon="user" onChange={this.handleChange} />
